...
int valore = 0;
int percentuale = 0;
char buffer[16]; // Aumentato per prevenire buffer overflow
...
int clienti = 0;
int profitto = 0;
// Rimossi i caratteri di escape non validi (\( e \))
String posizione = "Latitudine: (45° 42' 0.07'' N) Longitudine: (12° 15' 30.89'' E)";
...
void loop() {
  ...
  if (triggerStampa) {
    ...
    // snprintf è più sicuro di sprintf
    snprintf(buffer, sizeof(buffer), "%02d:%02d:%02d", ore, minuti, secondi);
    StaticJsonDocument<200> doc; // Nota: se usi ArduinoJson 7, cambia in JsonDocument doc;
